...
async function assertWithinStudentLimit({
  ageCategoryId,
  categories,
  centerId,
  excludeStudentId,
  ...
  const category = categories.find(
    (candidate) => candidate.id === ageCategoryId
  );
  if (!category) {
  ...
  if (count >= limit) {
    throw new Error(
      `${gender === "female" ? "Female" : "Male"} Student limit is full for this Age Category`
    );
  }
}
...
export const kalakritiStudentMutators = {
  create: defineMutator(
    ...
    async ({ tx, ctx, args }) => {
      ...
      await assertWithinStudentLimit({
