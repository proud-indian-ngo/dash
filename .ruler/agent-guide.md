...
- DO: When adding a new database table to `packages/db/src/schema/`, also add seed data in `packages/db/src/seed.ts` with 2-3 realistic records. All seed inserts must be idempotent (use `onConflictDoNothing()` or deterministic IDs).
