...
The app benchmark also opens the synthetic Team as a non-member volunteer. It verifies the denied Team result is empty and the route's event preload contains exactly the fixture's public event IDs. The fixture reports deterministic `publicEventIds` for this boundary check. This distinguishes unused denied-page preload work from private-event exposure.
