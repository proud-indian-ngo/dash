...
- **`packages/env/src/server.ts`** — server-only secrets. Includes `DATABASE_URL`, `BETTER_AUTH_SECRET` (min 32 chars), SMTP, R2 credentials, WhatsApp, Immich API keys. Optional `R2_ENDPOINT` / `R2_REGION` retarget the S3 client at MinIO (CI E2E).
