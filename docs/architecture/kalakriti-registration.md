...
Center detail pages expose vehicle assignments with capacity, driver contact fields, and notes. `kalakritiTransport.create`, `update`, and `transitionStatus` serialize writes through Edition and Center row locks, reject archived Editions and retired Centers, and audit the commands. Status advances only one step at a time: `planned` → `arrived_at_center` → `arrived_at_venue` → `departed_venue` → `completed`. Each transition appends the actor, timestamps, and prior/new status to `kalakriti_transport_status_history`.

Global/Edition administrators and Transport Leads manage transport Edition-wide. Transport Coordinators and Center-scoped Liaisons manage only their assigned Centers. Guardians can read their Center's transport but cannot mutate it. Transport-only staff can discover their permitted Centers without gaining Student or Entry registration-write permissions. The Center-scoped `transport_coordinator` responsibility uses the normal volunteer assignment workflow.
