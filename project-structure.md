...
| `routes/_app/kalakriti/$year/entries.tsx` | Center-scoped individual and group Competition Entry registration with live eligibility, limit, overlap, and capacity validation |
...
| `routes/kalakriti/$year/schedule.tsx` | Unauthenticated public Competition schedule for open, locked, live, and archived Editions; draft and unknown Editions return a safe unavailable state |
...
- **Projects**: Setup plus six browser projects — `super_admin`, `admin`, `finance_admin`, `volunteer`, `unoriented_volunteer`, and `unauthenticated`.
- **Auth state**: Global setup (`packages/e2e/global-setup.ts`) logs in the standard users and the active Kalakriti role actors, then saves their storage states to `packages/e2e/.auth/`. Feature tests reuse these sessions; the deliberately dormant external actor has no saved session.
