...
- **Kalakriti scale benchmark**: `tests/performance/kalakriti.spec.ts` uses `helpers/seed-kalakriti-performance.ts` in the isolated test database. Opt in with `KALAKRITI_PERFORMANCE=true`; query plans and row counts are attached to the Playwright report. See `docs/architecture/e2e-testing.md` for the command.
- **App scale benchmark**: `tests/performance/app.spec.ts` uses `helpers/seed-app-performance.ts` for Dashboard, Events and financial query profiling. Opt in with `APP_PERFORMANCE=true`; both benchmarks share `helpers/zero-performance.ts`.
