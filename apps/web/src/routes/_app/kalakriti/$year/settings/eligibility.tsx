...
export const Route = createFileRoute(
  "/_app/kalakriti/$year/settings/eligibility"
)({
  beforeLoad: ({ context }) => {
...
function KalakritiEligibilityPage() {
  ...
  const overlap = findKalakritiAgeCategoryOverlap(categoryRows);
  const missingLimits = categoryRows.filter(
    (category) => category.femaleStudentLimit + category.maleStudentLimit <= 0
  );
  ...
  return (
    <div className="space-y-6">
      ...
      (overlap || missingLimits.length > 0) ? (
        <Alert variant="destructive">
          <AlertTitle>Eligibility needs attention</AlertTitle>
          <AlertDescription>
            {overlap ? (
              <p>
                {overlap[0]} and {overlap[1]} have overlapping age ranges.
              </p>
            ) : null}
            {missingLimits.length > 0 ? (
              <p>
                {missingLimits.length} Age{" "}
                {missingLimits.length === 1
                  ? "Category has"
                  : "Categories have"}{" "}
                no Student limit.
              </p>
