...
export interface AttendanceByStudentDataPoint {
  name: string;
  present: number;
  rate: number;
  total: number;
...
export function AttendanceByStudentChart({
  data,
}: {
  data: AttendanceByStudentDataPoint[];
}) {
  if (data.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Low Attendance Students</CardTitle>
        </CardHeader>
              ...
              content={
                <ChartTooltipContent
                  formatter={(value, _name, props) => {
                    const p = (
                      props as { payload: AttendanceByStudentDataPoint }
                    ).payload;
