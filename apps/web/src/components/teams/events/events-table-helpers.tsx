...
import type { VirtualOccurrence } from "@pi-dash/shared/rrule-expand";
...
function expandSeriesEvent(
  ...
  const { occurrences, seriesStartDate, virtualOccs } = result;
  ...
  // Render every non-cancelled materialized exception, with no range filter.
  for (const exc of event.exceptions) {
    if (exc.cancelledAt) {
      continue;
    }
...
export function buildEventDisplayRows(
  ...
  for (const event of events) {
    ...
    const ruleUntil = parseRuleUntil(
      (event.recurrenceRule as { rrule: string }).rrule
    );
    const seriesRangeEnd = ruleUntil ?? rangeEndMs;
    rows.push(...expandSeriesEvent(event, event.startTime, seriesRangeEnd));
  }
