...
export function assembleKalakritiRegistrationDashboardProjection(
    ...
    .map((age) => {
      ...
      return {
        ...
        femaleStudentLimit: studentLimitsVisible
          ...
          : null,
        femaleStudents: studentLimitsVisible
          ? sum(students, (student) => student.femaleStudents)
          : null,
        id: age.id,
        maleStudentLimit: studentLimitsVisible ? age.maleStudentLimit : null,
        maleStudents: studentLimitsVisible
          ? sum(students, (student) => student.maleStudents)
          : null,
        name: age.name,
        participants: sum(entries, (entry) => entry.participants),
        registeredStudents:
  ...
  const perCenterStudentLimit = sum(
    rows.ages,
    (age) => age.femaleStudentLimit + age.maleStudentLimit
  );
  const centers = rows.centers.map((center) => {
    ...
    return {
      ...
      studentLimit: perCenterStudentLimit,
  ...
  return {
    ...
    totals: {
      ...
      studentLimit: studentLimitsVisible
        ? perCenterStudentLimit * rows.centers.length
        : null,
...
async function loadKalakritiRegistrationDashboardProjection({
  ...
  const ages = await tx
    .select({
      femaleStudentLimit: kalakritiAgeCategory.femaleStudentLimit,
      id: kalakritiAgeCategory.id,
      maleStudentLimit: kalakritiAgeCategory.maleStudentLimit,
