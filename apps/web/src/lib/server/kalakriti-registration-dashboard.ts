...
function competitionCondition(scope: KalakritiRegistrationDashboardScope) {
...
export function getKalakritiRegistrationDashboardProjection({
  editionId,
  scope,
}: {
  editionId: string;
  ...
  return db.transaction(
    ...
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Keeping the snapshot queries together makes the privacy boundary reviewable.
    async (tx) => {
        ...
        ? await tx
            ...
            .where(
              and(
                ...
                centerCondition(scope, kalakritiCenter.id)
              )
            )
      ...
      let categoryScopeCondition: SQL = sql`true`;
      if (
        scope.kind === "competition_category" &&
        scope.competitionCategoryIds
      ) {
        categoryScopeCondition = inArray(
        ...
        sessionIds.length > 0
          ? await tx
              .select({
                ...
                participants:
                  sql<number>`count(${kalakritiEntryMember.id})`.mapWith(
                    Number
                  ),
              ...
              .where(
                and(
                  ...
                  centerCondition(scope, kalakritiCompetitionEntry.centerId)
                )
              )
              .groupBy(
        ...
        ? await tx
            ...
            .where(
              and(
                ...
                centerCondition(scope, kalakritiStudent.centerId)
              )
            )
      ...
      const participants =
        !showCenters && sessionIds.length > 0
          ? await tx
              .select({
                ageCategoryId: kalakritiCompetitionSession.ageCategoryId,
                registeredStudents: countDistinct(
                  kalakritiEntryMember.studentId
                ),
              })
              ...
              .innerJoin(
                ...
                and(
                  ...
                  eq(
                    ...
                    kalakritiEntryMember.sessionId
                  )
                )
              )
              .where(
