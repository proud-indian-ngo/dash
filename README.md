...
| Kalakriti operation ledger | Record authorized live-Edition operations from JSON person identifiers or yearly IDs, with idempotent retries, scoped staff permissions, and Student/Entry deletion guards. Administrators and authorized Leads can add an audited correction note without undoing the recorded fact; meal undo remains a separate action. |
...
| Kalakriti Center transport | The Transport Lead and administrators manage vehicle assignments. Finalized Center scan stages update vehicle status; Guardians and Center Liaisons can view their own Center's transport details. |
| Kalakriti Food | A scoped roster shows eligible Students, Volunteers, and Guardians, with filters for every column and colored meal-status icons. Breakfast/lunch totals retain served history; admins and Food Leads can undo meal marks without deleting history. Volunteers also show check-in status in their roster. |
...
| Kalakriti Scan | Role-aware sidebar tabs cover Center transport, volunteer check-in, meals, and Competition attendance. Pickup allows absentees; later transport stages require every traveler. Meals require Student pickup, volunteer check-in, or active Guardian registration; Competition attendance requires pickup and a valid session. Students and Centers show transport status in their tables. |
